...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(request ->{
                    request.requestMatchers("/finstuff/v1/security/login", "/finstuff/v1/security/register").permitAll()
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(List.of("*")); // Разрешаем все origin
        configuration.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS"));
        configuration.setAllowedHeaders(List.of("*")); // Разрешаем все заголовки
        configuration.setAllowCredentials(false); // Для '*' credentials должны быть false
